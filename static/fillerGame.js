function printToOutput(text) {
  document.getElementById("gameOutput").value = text;
}
...
document.getElementById("closeQuartersButton").addEventListener("click", function () {
  printToOutput("");
  closeQuarters();
});
...
function winner(board) {
  ...
  let palyer1Count = 0;
  const player2 = board[board.length - 1][board.length - 1];function printBoard(board) {
    let boardString = '';
    for (let i = 0; i < board.length; i++) {
        for (let j = 0; j < board[i].length; j++) {
            boardString += board[i][j] + ' ';
...
function isValidCq(board, target) {
    if (target < 1) {
        return false;
    } else if (target > 6) {
        return false;
...
function createGameBoard(board) {
  ...
  for (let i = 0; i < board.length; i++) {
      for (let j = 0; j < board[i].length; j++) {
          ...
          cell.textContent = board[i][j]; // Set cell content based on your board data

          // Add event listener to cell for capturing moves
          cell.addEventListener("click", function () {
              // Handle the move when a cell is clicked
              // You can call your game logic functions here
              // Example: update the cell content and game output
              cell.textContent = "New Value"; // Replace with your logic
              printToOutput("New game output"); // Replace with your game output
          });
...
// Call the createGameBoard function to generate the grid
createGameBoard(/* Pass your game board data as a parameter */);